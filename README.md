...
Удобная программа с графическим интерфейсом — одна оболочка `WarlockTools_GUI.exe` вокруг конвертеров для игры *Warlock: Master of the Arcane* (Paradox Interactive). Никаких `.bat`-скриптов и консоли: просто перетащил файл — действие определилось само.

A friendly GUI — one shell `WarlockTools_GUI.exe` around the converters for *Warlock: Master of the Arcane* (Paradox Interactive). No `.bat` scripts or console: drop a file and the action is detected automatically.
...
Главная фишка: **взломан формат `locdata.md`** (`mdmp`) — тот самый файл локализации игры. Теперь его можно распаковать в обычный читаемый текст, отредактировать и собрать обратно в формат игры. Строки лежат в UTF-8.

The key feature: **the `locdata.md` format (`mdmp`) is cracked** — the game's localization file. It can now be unpacked into readable text, edited, and packed back into the game format. The string pool is UTF-8.
...
- **`locdata.md` → текст** и **текст → `locdata.md`** — правка локализации.
...
- **`locdata.md` → text** and **text → `locdata.md`** — localization editing.
